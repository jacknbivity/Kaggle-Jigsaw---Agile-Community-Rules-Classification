...
def get_df():
    ...
    df = pd.concat(merge, axis=0).drop_duplicates(ignore_index=True)
    
    # 创建规则到ID的映射
    unique_rules = sorted(df["rule"].unique())
    rule_to_id = {rule: idx for idx, rule in enumerate(unique_rules)}
    num_rules = len(unique_rules)
    
    # 添加辅助分类标签：如果违规则标签为rule_id，如果不违规则标签为num_rules（表示不违规类别）
    df["aux_label"] = df.apply(
        lambda row: rule_to_id[row["rule"]] if row["rule_violation"] == 1 else num_rules,
        axis=1
    )
    
    print(f"Number of unique rules: {num_rules}")
    print(f"Auxiliary classification: {num_rules} violation types + 1 non-violation = {num_rules + 1} classes")
    
    return df, rule_to_id, num_rules
